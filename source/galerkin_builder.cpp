...
GalerkinBuilder::GalerkinBuilder(const BuilderData &builder_data)
: data(builder_data)
{
    ...
    for (size_t K = 0; K < 18; ++K) {
        bessel_ws[K].resize(N, N);
        bessel_ws_s[K].resize(Ns, Ns);
    }
    m_h0_s.resize(Ns, Ns);
    m_h1_s.resize(Ns, Ns);
    m_v_s.resize(Ns, Ns);
...
inline void GalerkinBuilder::compute_coinciding(size_t i) throw() {
    ...
    if (k_real_positive) {
        complex_bessel::H1_01_i<Eigen::Dynamic>(ksqrtc.real() * m_v_norm[0], m_h0[0], m_h1[0], bessel_ws);
    } else {
        complex_bessel::H1_01_cplx<Eigen::Dynamic>(ksqrtc * m_v_norm[0], m_h0[0], m_h1[0]);
        m_h0[0] *= 1i;
        m_h1[0] *= 1i;
    }
...
inline void GalerkinBuilder::compute_adjacent(size_t i, size_t j, bool swap) throw() {
    ...
    if (k_real_positive) {
        complex_bessel::H1_01_i<Eigen::Dynamic>(ksqrtc.real() * m_v_norm[0], m_h0[0], m_h1[0], bessel_ws);
        complex_bessel::H1_01_i<Eigen::Dynamic>(ksqrtc.real() * m_v_norm[1], m_h0[1], m_h1[1], bessel_ws);
    } else for (K = 0; K < 2; ++K) {
        complex_bessel::H1_01_cplx<Eigen::Dynamic>(ksqrtc * m_v_norm[K], m_h0[K], m_h1[K]);
        m_h0[K] *= 1i;
        m_h1[K] *= 1i;
    }
    auto toc = chrono::high_resolution_clock::now();
...
inline void GalerkinBuilder::compute_general(size_t i, size_t j) throw() {
    ...
    auto tic = chrono::high_resolution_clock::now();
    if (k_real_positive) {
        complex_bessel::H1_01_i<Eigen::Dynamic>(ksqrtc.real() * m_v_norm_s, m_h0_s, m_h1_s, bessel_ws_s);
    } else {
        complex_bessel::H1_01_cplx<Eigen::Dynamic>(ksqrtc * m_v_norm_s, m_h0_s, m_h1_s);
        m_h0_s *= 1i;
        m_h1_s *= 1i;
    }
    auto toc = chrono::high_resolution_clock::now();
...
void GalerkinBuilder::double_layer_coinciding(int der) throw() {
    ...
    auto mask = (ksqrtca * v_norm) > eps_mach;
    double_layer_interaction_matrix.setZero();
    masked1[0] = mask.select(ksqrtc * h1 / v_norm, (v_norm > eps_mach).select(M_2_PI / v_norm2, m_zero));
    if (der > 0) {
...
void GalerkinBuilder::double_layer_adjacent(bool swap, int der, bool transp) throw() {
    ...
    for (K = 0; K < 2; ++K) {
        ...
        if (!transp) {
            ...
            auto mask = (ksqrtca * v_norm) > eps_mach;
            if (k_real_positive)
                masked1[K] = mask.select((ksqrtc.real() / v_norm) * h1, (v_norm > eps_mach).select(M_2_PI / v_norm2, m_zero));
            else masked1[K] = mask.select(ksqrtc * h1 / v_norm, (v_norm > eps_mach).select(M_2_PI / v_norm2, m_zero));
            if (der > 0)
        ...
        const auto &fg = swap ? ((transp ? K == 0 : K == 1) ? data.m_double_layer_adjacent_fg_swap_t
...
void GalerkinBuilder::double_layer_general(int der, bool transp) throw() {
    ...
    const auto &fg = transp ? data.m_double_layer_general_fg_t : data.m_double_layer_general_fg;
    if (!transp) {
        vdotn_s = data.m_wg * (m_v_s.real() * m_tangent_p_s.imag() - m_v_s.imag() * m_tangent_p_s.real()) * m_tangent_norm_s;
        auto mask = ksqrtca * m_v_norm_s > eps_mach;
        if (k_real_positive)
            masked1_s = mask.select((ksqrtc.real() / m_v_norm_s) * m_h1_s, (m_v_norm_s > eps_mach).select(M_2_PI / m_v_norm2_s, m_zero_s));
        else masked1_s = mask.select(ksqrtc * m_h1_s / m_v_norm_s, (m_v_norm_s > eps_mach).select(M_2_PI / m_v_norm2_s, m_zero_s));
        if (der > 0)
    ...
    } else vdotn_s = data.m_wg * (m_v_s.imag() * m_tangent_s.real() - m_v_s.real() * m_tangent_s.imag()) * m_tangent_p_norm_s;
    if (Qtrial == 2 && Qtest == 2 && der == 0) {
        masked1_hg_s = masked1_s * vdotn_s;
        double_layer_interaction_matrix(0, 0) = (fg.block(0, 0, N, N) * masked1_hg_s).sum();
        double_layer_interaction_matrix(1, 0) = (fg.block(N, 0, N, N) * masked1_hg_s).sum();
        double_layer_interaction_matrix(0, 1) = (fg.block(0, N, N, N) * masked1_hg_s).sum();
        double_layer_interaction_matrix(1, 1) = (fg.block(N, N, N, N) * masked1_hg_s).sum();
        return;
    }
    for (j = 0; j < Qtrial; ++j) {
        for (i = 0; i < Qtest; ++i) {
            cf_s = vdotn_s * fg.block(i * N, j * N, N, N);
            double_layer_interaction_matrix(i, j) = (cf_s * masked1_s).sum();
            if (der > 0)
                double_layer_der_interaction_matrix(i, j) = (cf_s * masked2_s).sum();
            if (der > 1)
                double_layer_der2_interaction_matrix(i, j) = (cf_s * masked3_s).sum();
        }
    }
    if (der > 0)
        double_layer_der_interaction_matrix *= k * c;
...
void GalerkinBuilder::hypersingular_coinciding(int der) throw() {
    ...
    const auto &v_norm = m_v_norm[0], &v_norm2 = m_v_norm2[0];
    tdottp = 2.0 * (tangent.imag() * tangent_p.imag() + tangent.real() * tangent_p.real()) * data.m_wc;
    auto mask = ksqrtca * v_norm > eps_mach;
    Eigen::ArrayXXcd h1_vnorm;
    hypersingular_interaction_matrix.setZero();
    masked1[0] = mask.select(h0, (v_norm > eps_mach).select(-M_2_PI * v_norm.log(), m_zero));
    if (der > 0) {
    ...
    for (K = 0; K < 2; ++K) {
        for (j = 0; j < Qtrial; ++j) {
            for (i = 0; i < Qtrial; ++i) {
                ...
                cf = data.m_wc * data.m_hypersingular_coinciding_fg_arc.block((K > 0 ? j : i) * N, (K > 0 ? i : j) * N, N, N) - kkc * temp;
...
void GalerkinBuilder::hypersingular_general(int der) throw() {
    ...
    tdottp_s = 2.0 * (tangent.imag() * tangent_p.imag() + tangent.real() * tangent_p.real()) * data.m_wg;
    auto mask = ksqrtca * v_norm > eps_mach;
    Eigen::ArrayXXcd h1_vnorm;
    masked1_s = mask.select(h0, (v_norm > eps_mach).select(-M_2_PI * v_norm.log(), m_zero_s));
    if (der > 0)
        h1_vnorm = m_h1_s * v_norm;
    for (j = 0; j < Qtrial; ++j) {
        for (i = 0; i < Qtrial; ++i) {
            temp_s = data.m_hypersingular_general_fg.block(i * N, j * N, N, N) * tdottp_s;
            cf_s = data.m_wg * data.m_hypersingular_general_fg_arc.block(i * N, j * N, N, N) - kkc * temp_s;
            hypersingular_interaction_matrix(i, j) = (cf_s * masked1_s).sum();
            if (der > 0)
                hypersingular_der_interaction_matrix(i, j) =
                    mask.select(h1_vnorm * cf_s + ksqrtc * h0 * temp_s, m_zero_s).sum();
            if (der > 1)
                hypersingular_der2_interaction_matrix(i, j) = mask.select(
                    (h0 * v_norm2 - h1_vnorm / ksqrtc) * cf_s - temp_s * (2.0 * ksqrtc * h1_vnorm - h0), m_zero_s).sum();
        }
    }
    if (der > 0)
        hypersingular_der_interaction_matrix *= -sqrtc;
...
void GalerkinBuilder::single_layer_coinciding(int der) throw() {
    ...
    ttp_norm = m_tangent_norm[0] * m_tangent_p_norm[0] * data.m_wc;
    auto mask = ksqrtca * v_norm > eps_mach;
...
void GalerkinBuilder::single_layer_adjacent(bool swap, int der) throw() {
    ...
    for (K = 0; K < 2; ++K) {
        ...
        auto mask = ksqrtca * v_norm > eps_mach;
        masked1[K] = mask.select(h0, (v_norm > eps_mach).select(-M_2_PI * v_norm.log(), m_zero));
        if (der > 0)
...
void GalerkinBuilder::single_layer_general(int der) throw() {
    ...
    ttp_norm_s = m_tangent_norm_s * m_tangent_p_norm_s * data.m_wg;
    auto mask = ksqrtca * v_norm > eps_mach;
    masked1_s = mask.select(h0, (v_norm > eps_mach).select(-M_2_PI * v_norm.log(), m_zero_s));
    if (der > 0)
    ...
    for (j = 0; j < Qtest; ++j) {
        for (i = 0; i < Qtest; ++i) {
            cf_s = ttp_norm_s * data.m_single_layer_general_fg.block(i * N, j * N, N, N);
            single_layer_interaction_matrix(i, j) = (cf_s * masked1_s).sum();
            if (der > 0)
                single_layer_der_interaction_matrix(i, j) = (cf_s * masked2_s).sum();
            if (der > 1)
                single_layer_der2_interaction_matrix(i, j) = (cf_s * masked3_s).sum();
        }
    }
    if (der > 0)
        single_layer_der_interaction_matrix *= -sqrtc;
...
void GalerkinBuilder::all_coinciding(int der) throw() {
    ...
    tdottp = 2.0 * (tangent.imag() * tangent_p.imag() + tangent.real() * tangent_p.real()) * data.m_wc;
    auto mask = (ksqrtca * v_norm) > eps_mach;
    auto mask2 = v_norm > eps_mach;
    if (k_real_positive)
        masked1[0] = mask.select((ksqrtc.real() / v_norm) * h1, mask2.select(M_2_PI / v_norm2, m_zero));
    else masked1[0] = mask.select(ksqrtc * h1 / v_norm, mask2.select(M_2_PI / v_norm2, m_zero));
    masked1_hg = mask.select(h0, mask2.select(-M_2_PI * v_norm.log(), m_zero));
    if (Q == 2 && k_real_positive && der == 0) {
        masked1[1] = masked1[0] * data.m_wc * m_tangent_p_norm[0] * (v.imag() * tangent.real() - v.real() * tangent.imag());
        masked1[0] *= -data.m_wc * m_tangent_norm[0] * (v.imag() * tangent_p.real() - v.real() * tangent_p.imag());
        double_layer_interaction_matrix(0, 0) = (data.m_double_layer_coinciding_fg.block(0, 0, N, N) * masked1[0] +
                                                 data.m_double_layer_coinciding_fg_t.block(0, 0, N, N) * masked1[1]).sum();
        double_layer_interaction_matrix(1, 0) = (data.m_double_layer_coinciding_fg.block(1, 0, N, N) * masked1[0] +
                                                 data.m_double_layer_coinciding_fg_t.block(1, 0, N, N) * masked1[1]).sum();
        double_layer_interaction_matrix(0, 1) = (data.m_double_layer_coinciding_fg.block(0, 1, N, N) * masked1[0] +
                                                 data.m_double_layer_coinciding_fg_t.block(0, 1, N, N) * masked1[1]).sum();
        double_layer_interaction_matrix(1, 1) = (data.m_double_layer_coinciding_fg.block(1, 1, N, N) * masked1[0] +
                                                 data.m_double_layer_coinciding_fg_t.block(1, 1, N, N) * masked1[1]).sum();
        hypersingular_interaction_matrix(0, 0) = 2. * ((data.m_wc * data.m_hypersingular_coinciding_fg_arc.block(0, 0, N, N) -
            kkc.real() * data.m_hypersingular_coinciding_fg.block(0, 0, N, N) * tdottp) * masked1_hg).sum();
        hypersingular_interaction_matrix(1, 1) = 2. * ((data.m_wc * data.m_hypersingular_coinciding_fg_arc.block(N, N, N, N) -
            kkc.real() * data.m_hypersingular_coinciding_fg.block(N, N, N, N) * tdottp) * masked1_hg).sum();
        hypersingular_interaction_matrix(1, 0) = hypersingular_interaction_matrix(0, 1) =
            ((data.m_wc * (data.m_hypersingular_coinciding_fg_arc.block(N, 0, N, N) + data.m_hypersingular_coinciding_fg_arc.block(0, N, N, N)) -
              kkc.real() * (data.m_hypersingular_coinciding_fg.block(N, 0, N, N) + data.m_hypersingular_coinciding_fg.block(0, N, N, N)) * tdottp) * masked1_hg).sum();
        masked1_hg *= m_tangent_norm[0] * m_tangent_p_norm[0] * data.m_wc;
        single_layer_interaction_matrix(0, 0) = 2. * (data.m_single_layer_coinciding_fg.block(0, 0, N, N) * masked1_hg).sum();
        single_layer_interaction_matrix(1, 1) = 2. * (data.m_single_layer_coinciding_fg.block(N, N, N, N) * masked1_hg).sum();
        single_layer_interaction_matrix(1, 0) = single_layer_interaction_matrix(0, 1) =
            ((data.m_single_layer_coinciding_fg.block(N, 0, N, N) + data.m_single_layer_coinciding_fg.block(0, N, N, N)) * masked1_hg).sum();
        return;
    }
    ttp_norm = m_tangent_norm[0] * m_tangent_p_norm[0] * data.m_wc;
    double_layer_interaction_matrix.setZero();
    ...
    for (K = 0; K < 2; ++K) {
        ...
        const auto &fg = K == 0 ? data.m_double_layer_coinciding_fg : data.m_double_layer_coinciding_fg_t;
        for (j = 0; j < Q; ++j) {
            for (i = 0; i < Q; ++i) {
...
void GalerkinBuilder::all_adjacent(bool swap, int der) throw() {
    ...
    for (K = 0; K < 2; ++K) {
        ...
        const auto &h1 = m_h1[K], &h0 = m_h0[K];
        tdottp = 2.0 * (tangent.imag() * tangent_p.imag() + tangent.real() * tangent_p.real()) * data.m_wa;
        vdotn = data.m_wa * (v.real() * tangent_p.imag() - v.imag() * tangent_p.real()) * m_tangent_norm[K];
        ttp_norm = m_tangent_norm[K] * m_tangent_p_norm[K] * data.m_wa;
        auto mask = (ksqrtca * v_norm) > eps_mach;
        auto mask2 = v_norm > eps_mach;
        masked1_hg = mask.select(h0, mask2.select(-M_2_PI * v_norm.log(), m_zero));
        ...
        const auto &fg = swap ? (K == 1 ? data.m_double_layer_adjacent_fg_swap_t : data.m_double_layer_adjacent_fg_swap)
...
void GalerkinBuilder::all_general(int der) throw() {
    ...
    tdottp_s = 2.0 * (m_tangent_s.imag() * m_tangent_p_s.imag() + m_tangent_s.real() * m_tangent_p_s.real()) * data.m_wg;
    auto mask = ksqrtca * m_v_norm_s > eps_mach;
    auto mask2 = m_v_norm_s > eps_mach;
    if (k_real_positive)
        masked1_s = mask.select((ksqrtc.real() / m_v_norm_s) * m_h1_s, mask2.select(M_2_PI / m_v_norm2_s, m_zero_s));
    else masked1_s = mask.select(ksqrtc * m_h1_s / m_v_norm_s, mask2.select(M_2_PI / m_v_norm2_s, m_zero_s));
    masked1_hg_s = mask.select(m_h0_s, mask2.select(-M_2_PI * m_v_norm_s.log(), m_zero_s));
    if (Q == 2 && k_real_positive && der == 0) {
        tdottp_s *= kkc.real();
        hypersingular_interaction_matrix(0, 0) = ((data.m_wg * data.m_hypersingular_general_fg_arc.block(0, 0, N, N) -
            data.m_hypersingular_general_fg.block(0, 0, N, N) * tdottp_s) * masked1_hg_s).sum();
        hypersingular_interaction_matrix(1, 0) = ((data.m_wg * data.m_hypersingular_general_fg_arc.block(N, 0, N, N) -
            data.m_hypersingular_general_fg.block(N, 0, N, N) * tdottp_s) * masked1_hg_s).sum();
        hypersingular_interaction_matrix(0, 1) = ((data.m_wg * data.m_hypersingular_general_fg_arc.block(0, N, N, N) -
            data.m_hypersingular_general_fg.block(0, N, N, N) * tdottp_s) * masked1_hg_s).sum();
        hypersingular_interaction_matrix(1, 1) = ((data.m_wg * data.m_hypersingular_general_fg_arc.block(N, N, N, N) -
            data.m_hypersingular_general_fg.block(N, N, N, N) * tdottp_s) * masked1_hg_s).sum();
        masked1_hg_s *= m_tangent_norm_s * m_tangent_p_norm_s * data.m_wg;
        single_layer_interaction_matrix(0, 0) = (data.m_single_layer_general_fg.block(0, 0, N, N) * masked1_hg_s).sum();
        single_layer_interaction_matrix(1, 0) = (data.m_single_layer_general_fg.block(N, 0, N, N) * masked1_hg_s).sum();
        single_layer_interaction_matrix(0, 1) = (data.m_single_layer_general_fg.block(0, N, N, N) * masked1_hg_s).sum();
        single_layer_interaction_matrix(1, 1) = (data.m_single_layer_general_fg.block(N, N, N, N) * masked1_hg_s).sum();
        masked1_hg_s = masked1_s * data.m_wg * (m_v_s.real() * m_tangent_p_s.imag() - m_v_s.imag() * m_tangent_p_s.real()) * m_tangent_norm_s;
        double_layer_interaction_matrix(0, 0) = (data.m_double_layer_general_fg.block(0, 0, N, N) * masked1_hg_s).sum();
        double_layer_interaction_matrix(1, 0) = (data.m_double_layer_general_fg.block(N, 0, N, N) * masked1_hg_s).sum();
        double_layer_interaction_matrix(0, 1) = (data.m_double_layer_general_fg.block(0, N, N, N) * masked1_hg_s).sum();
        double_layer_interaction_matrix(1, 1) = (data.m_double_layer_general_fg.block(N, N, N, N) * masked1_hg_s).sum();
        return;
    }
    ttp_norm_s = m_tangent_norm_s * m_tangent_p_norm_s * data.m_wg;
    vdotn_s = data.m_wg * (m_v_s.real() * m_tangent_p_s.imag() - m_v_s.imag() * m_tangent_p_s.real()) * m_tangent_norm_s;
    if (der > 0) {
        masked2_s = mask.select(m_h0_s, m_zero_s);
        masked2_g_s = mask.select(m_h1_s * m_v_norm_s, m_zero_s);
    }
    if (der > 1) {
        masked3_s = masked2_s - ksqrtc * masked2_g_s;
        masked3_g_s = masked2_g_s / ksqrtc - masked2_s * m_v_norm2_s;
    }
    for (size_t j = 0; j < Q; ++j) {
        for (size_t i = 0; i < Q; ++i) {
            cfr1_s = vdotn_s * data.m_double_layer_general_fg.block(i * N, j * N, N, N);
            cfr2_s = ttp_norm_s * data.m_single_layer_general_fg.block(i * N, j * N, N, N);
            temp_s = data.m_hypersingular_general_fg.block(i * N, j * N, N, N) * tdottp_s;
            if (k_real_positive)
                cfr3_s = data.m_wg * data.m_hypersingular_general_fg_arc.block(i * N, j * N, N, N) - kkc.real() * temp_s;
            else cf_s = data.m_wg * data.m_hypersingular_general_fg_arc.block(i * N, j * N, N, N) - kkc * temp_s;
            double_layer_interaction_matrix(i, j) = (cfr1_s * masked1_s).sum();
            single_layer_interaction_matrix(i, j) = (cfr2_s * masked1_hg_s).sum();
            if (k_real_positive)
                hypersingular_interaction_matrix(i, j) = (cfr3_s * masked1_hg_s).sum();
            else hypersingular_interaction_matrix(i, j) = (cf_s * masked1_hg_s).sum();
            if (der > 0) {
                double_layer_der_interaction_matrix(i, j) = (cfr1_s * masked2_s).sum();
                single_layer_der_interaction_matrix(i, j) = (cfr2_s * masked2_g_s).sum();
                if (k_real_positive)
                    hypersingular_der_interaction_matrix(i, j) = (cfr3_s * masked2_g_s + (ksqrtc.real() * temp_s) * masked2_s).sum();
                else hypersingular_der_interaction_matrix(i, j) = (masked2_g_s * cf_s + ksqrtc * masked2_s * temp_s).sum();
            }
            if (der > 1) {
                double_layer_der2_interaction_matrix(i, j) = (cfr1_s * masked3_s).sum();
                single_layer_der2_interaction_matrix(i, j) = (cfr2_s * masked3_g_s).sum();
                if (k_real_positive)
                    hypersingular_der2_interaction_matrix(i, j) = -(cfr3_s * masked3_g_s + temp_s * (masked2_s - 2.0 * masked3_s)).sum();
                else hypersingular_der2_interaction_matrix(i, j) = -(masked3_g_s * cf_s + temp_s * (masked2_s - 2.0 * masked3_s)).sum();
            }
        }
    }
    if (der > 0) {
...
unsigned int GalerkinBuilder::getInteractionMatrixAssemblyTime() {
    auto ret = interaction_matrix_assembly_time - hankel_computation_time;
