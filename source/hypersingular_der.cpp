            ...
            for (int j = 0; j < Q; ++j) {
                ...
                auto integrand = [&](double s, double t) {
                    ...
                    if ( abs(k*sqrt(c))*(pi[s]-pi_p[t]).norm() > epsilon ) {
                        result = (complex_bessel::HankelH1(1,k*sqrt(c)*(pi[s]-pi_p[t]).norm())*(pi[s]-pi_p[t]).norm()*(F_arc(t) * G_arc(s) - k*sqrt(c)* k*sqrt(c)* F(t) * G(s) * normal.dot(normal_p))
                                + complex_bessel::HankelH1(0,k*sqrt(c)*(pi[s]-pi_p[t]).norm())*(2.0*k*sqrt(c)*F(t)*G(s)*normal.dot(normal_p)));
                    }
                    return result;
                };
                ...
                auto integrand = [&](double s, double t) {
                    ...
                    if (swap){
                        if ( abs(k*sqrt(c))*(pi[s]-pi_p.swapped_op(t)).norm() > epsilon ) {
                            result = (complex_bessel::HankelH1(1,k*sqrt(c)*(pi[s]-pi_p.swapped_op(t)).norm())*(pi[s]-pi_p.swapped_op(t)).norm()*(F_arc(t) * G_arc(s) - k*sqrt(c)* k*sqrt(c)* F(t) * G(s) * normal.dot(normal_p))
                                            + complex_bessel::HankelH1(0,k*sqrt(c)*(pi[s]-pi_p.swapped_op(t)).norm())*(2.0*k*sqrt(c)*F(t)*G(s)*normal.dot(normal_p)));
                        }
                    }else {
                        if ( abs(k*sqrt(c))*(pi.swapped_op(s)-pi_p[t]).norm() > epsilon ) {
                            result = (complex_bessel::HankelH1(1,k*sqrt(c)*(pi.swapped_op(s)-pi_p[t]).norm())*(pi.swapped_op(s)-pi_p[t]).norm()*(F_arc(t) * G_arc(s) - k*sqrt(c)* k*sqrt(c)* F(t) * G(s) * normal.dot(normal_p))
                                            + complex_bessel::HankelH1(0,k*sqrt(c)*(pi.swapped_op(s)-pi_p[t]).norm())*(2.0*k*sqrt(c)*F(t)*G(s)*normal.dot(normal_p)));
                        }
                    }
                    return result;
                };
